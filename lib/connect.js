...
export default function connect(mapState = defaultMapStateToProps, mapDispatch = defaultMapDispatchToProps, preLoad = true) {
  ...
  return function wrapWithConnect(pageConfig, target = Targets.Wechat.name) {
    ...
    if (pageConfig.isComponent) {
      ...
      _didUnmount = pageConfig[Targets[target].disconnectComponenentLife]
    } else {
      _didMount = pageConfig[Targets[target].conectPageLife]
    ...
    if (pageConfig.isComponent) {
      return { ...pageConfig, methods: { ...pageConfig.methods, ...mapDispatch(store.dispatch, store.getState()) }, [Targets[target].connectComponentLife]: didMount, [Targets[target].disconnectComponenentLife]: didUnmount }
    } else {
      return { ...pageConfig, ...mapDispatch(store.dispatch, store.getState()), [Targets[target].conectPageLife]: didMount, [Targets[target].disconnectPageLife]: didUnmount }
    }
  }
}
